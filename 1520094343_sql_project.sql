...
SELECT CONCAT(m.surname, ',', m.firstname) as member_name,
              f.name as facid_name,
              CASE WHEN m.memid = 0 AND f.guestcost*b.slots > 30 THEN f.guestcost*b.slots
                        WHEN  f.membercost*b.slots >30 THEN  f.membercost*b.slots END AS cost
   FROM Bookings b
...
SELECT SUM(cost) AS total_revenue,
              name
FROM 
(SELECT CASE WHEN memid = 0 THEN guestcost*slots
